...
### 1-1. 브랜치 생성
- [x] `git checkout -b spec-x-native-feature-usage` (브랜치 이름 = spec 디렉토리 이름)
- [x] 계획 산출물(spec/plan/task + queue 등록분) 초기 커밋 `e350cdc` (`docs(...): spec/plan/task 계획 산출물 작성`)
...
- [x] `docs/native-feature-usage.md` 작성 — §1 tier 정의 + 단일출처 원칙 / §2 상황-우선 표 / §3 tier 2·3 조건 요약 / §4 경계 각주
- [x] 정본 대조: 1·2·3단계(15종) + 보류(/batch) + N-A(/powerup·/radio) 전 기능 수록 + ADR-007 조건 일치 확인
- [x] `docs/decisions/ADR-007-native-feature-adoption-policy.md` Related 절에 본 문서 포인터 추가
- [x] Commit `4907928`: `docs(spec-x-native-feature-usage): 네이티브 기능 사용 playbook 신설 + ADR-007 링크`
...
- [x] **Ship Commit**: `docs(spec-x-native-feature-usage): ship walkthrough and pr description`
- [x] **Push**: `git push -u origin spec-x-native-feature-usage`
