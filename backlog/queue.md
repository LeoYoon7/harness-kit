...
- ~~**`test-two-tier-loading.sh` Check 4 FAIL (fragment 단어 수 초과)**~~ → ✓ **spec-x-fragment-two-tier-restore (#57)** 로 해소 (2026-06-16): 해소안 ①(상세 이전) 채택 — fragment 3095w→113w (핵심 규칙 요약만), 알림/선택지 규약은 tier-2 `agent/notify.md`(hk-align 로딩), 패턴은 `agent/align.md` 로 이전. test-two-tier 10/10 PASS.
...
- ~~**sdd 내부 main fallback 설정화** + ADR `review-base-resolution-chain`~~ → ✓ **spec-x-defaultbranch-consistency (#58)** (2026-06-16): `_resolve_base_branch()` 헬퍼(체인 + 2단 ref-fallback) + **ADR-015** 작성. sdd 517/1941 대체.
- ~~**hk-ship PR_BASE·governance main 전제 재검토**~~ → ✓ **spec-x-defaultbranch-consistency (#58)**: hk-ship PR_BASE/checkout base 를 defaultBranch 로 + constitution §3.2/§3.3 "always main" → "defaultBranch (기본 main)" 개정.
