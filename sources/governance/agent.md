...
**Context offloading (orchestrator–worker)** — the main session is a *context orchestrator*. When delegating: ① **delegate** token-heavy / polluting labor (multi-file impl, broad search; boundary → §6.7 threshold), keeping judgment and verification on main; ② inject a **scoped slice** (files, expected behavior, test command, commit format), not full history; ③ the worker returns a **distilled contract** (commit SHA, test + completion status [full/partial/failed], decisions), never the transcript; ④ orchestrator **retains verification** (on failure → §7 Hard Stop); ⑤ **fan out** independent jobs (→ §6.7). Applies always; director mode (→ §6.8) toggles delegation *aggressiveness*, not on/off. Rationale: ADR-010.
