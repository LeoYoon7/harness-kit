...
| `/hk-code-review` vs native `/code-review` (사용자 지적) | 동일(중복) / 다름(보완) | **다름** | 실제 구현 확인 — hk 는 spec대비검증+테스트커버리지, native 는 effort·`--fix`·`--comment`. "도입 불요" 철회 → 보조 도입 |
| `/radio`·`/powerup` 등급 (사용자 지적) | ✅ Go / N/A 무관 | **N/A** | BGM·학습은 거버넌스 무관 — 도입 논의 대상 아님 |
| 사용 제한 차원 (사용자 지적) | 산발 언급 / 축 신설 | **축 H 신설** | 클라우드/유료 가용성 제약 체계화 (§4.5 매트릭스) |
...
- **`/code-review` 기본형은 중복이 아니라 보완 (정정)** — 당초 `/hk-code-review`(Opus)와 역할 동일 → 도입 불요로 오판. 사용자 지적으로 실제 구현 확인 결과, native 는 effort/`--fix`/`--comment` 가 고유하고 hk 는 spec대비검증·테스트커버리지가 고유 → 보완적. *skill 한 줄 설명만 보고 표면적으로 묶은 것이 원인*.
- **순수 부가 기능의 분류 함정** — `/radio`·`/powerup` 을 "✅ Go"로 분류했으나, BGM·학습은 *도입/미도입을 논할 대상이 아닌* 거버넌스 무관 항목. "적합도 등급"에 모든 기능을 욱여넣으려다 발생한 과분류. N/A 등급 신설로 해소.
