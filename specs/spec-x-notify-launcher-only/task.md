...
- [ ] Commit: `refactor(spec-x-notify-launcher-only): silent-skip notify dispatcher when launcher omits channel`
...
- [ ] **walkthrough.md 작성** — 결정 근거 + 수동 검증 결과 + sources/.harness-kit diff 0 줄 증거
- [ ] **pr_description.md 작성** — 템플릿 준수 (요약 / 변경 / 검증 / 리스크)
- [ ] **Ship Commit**: `docs(spec-x-notify-launcher-only): ship walkthrough and pr description`
- [ ] **Push**: `git push -u origin spec-x-notify-launcher-only`
- [ ] **PR 생성**: 에이전트가 `/hk-pr-gh` 로 생성
